      ...
      <div className="max-w-5xl mx-auto px-6 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
          ...
          <ContactCard
            icon={<MailIcon />}
            label="Email Us"
            value="viharahospitality@gmail.com"
            href="mailto:viharahospitality@gmail.com"
            small
          />
          <ContactCard
            icon={<InstagramIcon />}
...
function ContactCard({
  ...
  return (
    <a
      ...
      className="bg-[rgba(217,181,157,0.1)] border border-[rgba(217,181,157,0.25)] rounded-sm px-6 py-7 flex flex-col items-center gap-3 hover:bg-[rgba(217,181,157,0.2)] transition-colors duration-300 group"
    >
      <span className="text-[#D9B59D] group-hover:scale-110 transition-transform duration-300">{icon}</span>
      <p className="text-[#DCD7CD] font-['Jost'] text-xs tracking-widest uppercase opacity-70">{label}</p>
      <p className={`text-white font-['Jost'] font-medium text-center leading-tight ${small ? 'text-sm' : 'text-base'}`}>
        {value}
      </p>
    </a>
  );
